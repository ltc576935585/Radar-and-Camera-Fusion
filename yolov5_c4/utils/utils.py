...
def get_latest_run(search_dir='./runs'):
    ...
    return max(last_list, key=os.path.getctime)#文件的创建时间
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    color = color or [random.randint(0, 255) for _ in range(3)]##改
    ...
    if label:
        ...
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 1, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
