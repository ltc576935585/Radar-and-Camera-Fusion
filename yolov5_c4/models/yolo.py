...
class Model(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False):#(bs,3,H,W)
        # print('a')
        if augment:###5通道  不能用 augment  需要改
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            #     '''
            #     x_radar=nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))(x_radar)
            #     x=torch.cat((x,x_radar),dim=1)

            z.append([m.type,m.i,x])
            y.append(x if m.i in self.save else None)  # save output self.save=<class 'list'>: [4, 6, 10, 14, 17, 18, 21, 22]
        # for i in range(26):
        #     print(z[i].size())
        if profile:
...
def parse_model(md, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(md['backbone'] + md['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv,Conv_crf1,Conv_crf2,Conv_crf3,Conv_crf4, Bottleneck, SPP, DWConv, MixConv2d, Focus, Focus_crf,CrossConv, BottleneckCSP,BottleneckCSP_crf1,BottleneckCSP_crf2,BottleneckCSP_crf3, C3]:
            c1, c2 = ch[f], args[0]
